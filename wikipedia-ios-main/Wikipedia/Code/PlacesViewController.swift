...
class PlacesViewController: ViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate, HintPresenting {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let appdelegate = appdelegate, appdelegate.universalLink == nil {
            locationManager.startMonitoringLocation()
        }else {
            guard let link = appdelegate?.universalLink else { return }
            let activity = NSUserActivity.wmf_activity(forWikipediaScheme: link)
            guard  let webPageUrl = activity?.webpageURL else { return }
            moveMapToCoordinate(webPageUrl)
            self.appdelegate?.universalLink = nil;
        }
        mapView.showsUserLocation = true
    }
    
    @objc public func moveMapToCoordinate(_ url: URL) {
        ...
        let lat = paths[paths.count-2]
        guard let long = paths.last, let longDouble = Double(long) else { return }
        guard let latDouble = Double(lat) else { return }
        zoomAndPanMapView(toLocation: CLLocation(latitude:latDouble, longitude: longDouble))
    }
    ...
    func promptForLocationAccess() {
        ...
        let enableLocationButtonTapHandler: ScrollableEducationPanelButtonTapHandler = { _ in
            ...
            self.presentedViewController?.dismiss(animated: true, completion: {
                ...
                if let appdelegate = self.appdelegate, appdelegate.universalLink == nil {
    ...
    @objc public func showArticleURL(_ articleURL: URL) {
        guard let article = dataStore.fetchArticle(with: articleURL), let title = articleURL.wmf_title,
            ...
            self.moveMapToCoordinate(articleURL)
            return
        }
